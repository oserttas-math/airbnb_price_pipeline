...
def go(args):
    """
    Train and Evaluate the Random Forest Regressor Algorithm
    """
    ...
    with open(args.rf_config) as file_processor:
        rf_config = json.load(file_processor)
    ...
    X_regressors = pd.read_csv(trainval_local_path)
    y_depvar = X_regressors.pop("price")  # this removes the column "price" from X and puts it into y

    logger.info("Minimum price: %s, Maximum price: %s",
        y_depvar.min(), y_depvar.max()
    )

    X_train, X_val, y_train, y_val = train_test_split(
        X_regressors, y_depvar,
        test_size=args.val_size,
        stratify=X_regressors[args.stratify_by],
    ...
    mlflow.sklearn.save_model(
        sk_pipe,
        path=export_path,  # random_forest_dir
    ...
    model_artifact = wandb.Artifact(
        ...
        type="model_export",
        description="Random Forest Regressor Pipeline",
        metadata=rf_config,
    )
...
def plot_feature_importance(pipe, feat_names):
    ...
    feat_imp = pipe["random_forest_reg"].feature_importances_[: len(feat_names)-1]
    # For the NLP feature we sum across all the TF-IDF dimensions into a global
    # NLP importance
    nlp_importance = sum(pipe["random_forest_reg"].feature_importances_[len(feat_names) - 1:])
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    random_forest_reg = RandomForestRegressor(**rf_config)
